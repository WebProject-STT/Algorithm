...
'''
NxM 크기의 배열 A의 값은 각 행의 합 중 최솟값
배열 회전 연산은 (r,c,s)로 이루어져있다.
r행 c열을 둘러싼 정사각형을 시계 방향으로 돌리는데
가장 왼쪽 윗칸이 r-s행, c-s열, 가장 오른쪽 아랫칸이 r+s행, c+s행인 정사각형을 돌린다.
여기서, s가 2일 경우
가장 왼쪽 윗칸이 r-s+1행, c-s+1열, 가장 오른쪽 아랫칸이 r+s-1행, c+s-1열인 정사각형도 돌려야한다.
회전 연산 수행 후 배열 A의 최솟값을 출력한다.
참고로, 회전 연산의 순서에 따라 배열 A의 최솟값은 달라질 수 있다.
---
사각형을 돌리는 회전 연산 함수를 생성한다.
가장 왼쪽 윗칸이 r-s+i, c-s+i이고, 가장 오른쪽 아랫칸이 r+s-i, c+s-i인 사각형에 대해 함수를 수행한다.
여기서 i는 0 <= i < s의 범위를 가진다.
또한, r과 c는 1부터 시작이므로 실제 연산 시에는 r-1, c-1에 대해 수행해야 한다.
회전 연산은 순서에 영향을 받으므로 연산 개수 K의 모든 순열에 대해 함수를 수행한다.
그리고 그 중 가장 작은 배열 A의 값을 출력한다.
'''
